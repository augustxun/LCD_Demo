...
void analyze_tremor(float32_t* fft_output_x, float32_t* fft_output_y, float32_t* fft_output_z, float& tremor_strength, bool& tremor_detected) {
    ...
    for (int i = TREMOR_FREQ_LOW; i <= TREMOR_FREQ_HIGH; i++) {
        ...
        if (combined_strength > THRESHOLD_LOW) { // 检查强度是否超过阈值 (去抖)
            tremor_detected = true; // 超过则检测到震颤，改震颤检测状态为是
...
void update_lcd_display(bool tremor_detected, float tremor_strength) {
    if (tremor_detected) {
        ...
        lcd.SetBackColor(LCD_COLOR_RED);  // 如果检测到震颤，将背景设为红色
        lcd.SetTextColor(LCD_COLOR_WHITE);  // 文字颜色设为白色
        lcd.DisplayStringAt(0, LINE(5), (uint8_t *)"Tremor Detected!", CENTER_MODE);  // 显示检测到震颤
        char buf[32];
        snprintf(buf, 32, "Strength: %.2f Hz", tremor_strength);  // 震颤强度保留至小数点后两位输出
        lcd.DisplayStringAt(0, LINE(6), (uint8_t *)buf, CENTER_MODE);  // 显示震颤强度文字
    } else {
        lcd.Clear(LCD_COLOR_WHITE);
        lcd.SetBackColor(LCD_COLOR_GREEN);  // 如果未检测到震颤，则将背景设为绿色
        lcd.SetTextColor(LCD_COLOR_BLACK);  // 文字颜色设为黑色
        lcd.DisplayStringAt(0, LINE(5), (uint8_t *)"No Tremor", CENTER_MODE);  // 显示未检测到震颤
    }
}
...
int main() {
    ...
    while (1) {
        ...
        fft_input_x[2 * cycle] = (float)filtered_gx;  // 存储处理后的x轴数据
        fft_input_y[2 * cycle] = (float)filtered_gy;  // 存储处理后的y轴数据
        fft_input_z[2 * cycle] = (float)filtered_gz;  // 存储处理后的z轴数据
        ...
        if (cycle==255) {
            ...
            float tremor_strength=0.0; bool tremor_detected;
